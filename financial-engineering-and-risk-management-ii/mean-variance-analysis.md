...
Recall: Efficient frontier = line through the points $$(0, r_f), (\sigma_m, \mu_m)$$ 

Slope of the capital market line

$$m_{CML} = \cfrac{\mu_m - r_f}{\sigma_m} = \text{maximum achievable Sharpe ratio}$$ 

$$m_{CML}$$ is frequently called the **price of risk**. It is used to compare projects.

$$\bar{r} = r_f + \sigma \cdot m_{CML}$$ 

### Inferring Asset Returns from Market Returns

An asset is a portfolio: $$\text{asset }j \equiv \bm{x}^{(j)} = (0,...,0,1,0,...,0)^{\top},$$ 1 in the j-th position.

Diversity between $$\bm{x}^{(j)}$$ and market portfolio $$\bm{x}^{(m)} : \gamma \bm{x}^{(j)} + (1-\gamma) \bm{x}^{(m)}$$ 

* return $$\mu_{\gamma} = \gamma \mu_j + (1 - \gamma) \mu_m$$ 
* volatility $$\sigma_{\gamma} = \sqrt{\gamma^2 \sigma_j^2 + (1-\gamma)^2 \sigma_m^2 + 2 \sigma_{jm} \gamma(1-\gamma)}$$ 

![](../.gitbook/assets/image%20%2857%29.png)

All three curves are tangent at $$(\sigma_m, r_m)$$ . 

* Slope of the capital market line
  * $$m_{CML} = \cfrac{\mu_m - r_f}{\sigma_m}$$ 
* Slope of the frontier generated by asset j and market portfolio $$\bm{x}^{(m)}$$ 
  * $$\cfrac{d\mu_{\gamma}}{d\sigma_{\gamma}} = \cfrac{\cfrac{d\mu_{\gamma}}{d\gamma}}{\cfrac{d\sigma_\gamma}{d\gamma}}$$
  * $$\cfrac{d\mu_\gamma}{d\sigma_\gamma} | _{\gamma = 0} = \cfrac{\mu_j - \mu_m}{\cfrac{\sigma_{jm} - \sigma^2_m}{\sigma_m}}$$ 
* Equating slopes at $$\gamma = 0$$ we get the following result:
  * $$\mu_j - r_f = \bigg( \cfrac{\sigma_{jm}}{\sigma^2_m} \bigg) (\mu_m - r_f)$$ 
  * where the beta of asset j is $$\beta_j = \cfrac{\sigma_{jm}}{\sigma_m^2}$$ 

This pricing formula is called the **Capital Asset Pricing Model \(CAPM\)**

### Connecting CAPM to Regression

Regress the excess return $$r_j - r_f$$ of asset j on the excess market return $$r_m - r_f$$ 

$$(r_j - r_f) = \alpha + \beta (r_m  - r_f) + \epsilon_j$$ 
